...
export class SocialContentFormatterService {
  ...
  private formatForLinkedIn(
    ...
    const lines = [
      ...
      salaryText,
      payload.skills && payload.skills.length > 0 ? `🛠 Key Skills: ${payload.skills.slice(0, 5).join(', ')}` : null,
      '',
      '👉 Apply now through JobBaskets:',
      jobUrl,
      '',
      hashtags.join(' '),
  ...
  private formatForFacebook(
    ...
    const lines = [
      `🔥 New Job Opening: ${payload.title}!`,
      `🏢 ${payload.company_name} is looking for talent.`,
      `📍 ${locations}${workType}`,
      salaryText,
      '',
      'Apply here: ' + jobUrl,
  ...
  private formatForInstagram(
    ...
    const lines = [
      `📢 HIRING ALERT: ${payload.title}`,
      `🏢 ${payload.company_name}`,
      `📍 ${locations}${workType}`,
      salaryText,
      '',
      '🔗 Link in bio / visit JobBaskets to apply directly!',
      '',
      '.',
      '.',
      hashtags.join(' '),
